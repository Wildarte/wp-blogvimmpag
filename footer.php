<?php $cor_text_footer = get_option('show_cor_text_footer'); ?>
<style>
    footer.footer h3, .footer_col_1 p, ul.footer_contato li a, .footer_copy p{
        color: <?= $cor_text_footer; ?>;
    }

</style>
    <footer class="footer container-full" style="background-color: <?= get_option('show_cor_footer'); ?>">
        ...
        <div class="footer_copy" style="background-color: <?= get_option('show_cor_footer'); ?>">
            <p style="padding: 15px; background-color: rgba(0,0,0, 0.5)"><?= date('Y') ?> - Todos os direitos reservados</p>
        </div>
    </footer>
    <style>
    ...
    <?php
        $num_whastapp = get_option('show_num_whatsapp');
        $onoff_whatsap = get_option('show_onoff_whatsapp');
        if($num_whastapp != "" && $onoff_whatsap != "on"):
    ?>
    <a class="btn_whatsapp" href="https://api.whatsapp.com/send?phone=<?= $num_whastapp; ?>&amp;text=" target="_blank" style="position:fixed;bottom:15px;right:15px; z-index: 1000">
